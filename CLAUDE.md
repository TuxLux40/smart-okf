...
- Everything else (enrichment gate, derive/dream reasoning, API) is optional/later, not built — see the 2026-07-17/18 scope amendments at the top of `docs/DESIGN.md` before assuming a component exists or resurrecting cut scope (watcher, per-file companions, Streamlit, FastAPI, MCP server). `index.md` generation specifically was dropped as redundant, not deferred — see `docs/OKF_SPEC.md`'s "Index files" section.
